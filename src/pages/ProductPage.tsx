...
import { fetchProductBySlug, selectProductBySlug } from "../reducers/productsSlice";
    ...
    return (
        ...
        >
            <Grid
                ...
                maxWidth="lg"
                columns={12}
                sx={{ paddingTop: "24px", marginBottom: "16px", marginX: " auto" }}
            >
                <Grid
                    item
                    xs={12}
                    md={6}
                    sx={{ paddingX: "24px", alignItems: "center", flexDirection: "column", display: "flex" }}
                >
                    <Box>
                        <img
                            src={BASE_IMAGE_URL + product.imagesUrl[selectedProductUrl]}
                            style={{ objectFit: "contain", height: "350px", width: "350px" }}
                            alt={"Main Image"}
                        />
                    </Box>
                    <Box
                    ...
                    >
                        {product.imagesUrl.map((_, index) => {
                            return (
                                <Box
                                    key={index}
                                    onClick={() => setSelectedProductUrl(index)}
                                    sx={{
                                        border: "1px solid",
                                        borderColor: [
                                            selectedProductUrl === index ? "secondary.main" : "primary.light",
                                        ],
                                        borderRadius: "10px",
                                        width: "64px",
                                        height: "64px",
                                ...
                                >
                                    <Avatar sx={{ height: "40px" }} variant="square">
                                        <img
                                            src={BASE_IMAGE_URL + product.imagesUrl[index]}
                                            alt=""
                ...
                <Grid item xs={12} md={6} sx={{ paddingLeft: "10px" }}>
                    ...
                    <Box sx={{ marginBottom: "20px" }}>
                        <Typography
                            variant="h2"
                            sx={{ fontSize: "25px", fontWeight: "600", color: "secondary.main", marginBottom: "15x" }}
                        >
                        ...
                        {existsInCart ? (
                            ...
                            >
                                <Button
                                    ...
                                    color="secondary"
                                    onClick={() =>
                                        dispatch(changeQuantity({ id: cartItem.id, quantity: cartItem.quantity - 1 }))
                                    }
                                >
                            ...
                            <Button
                                variant="contained"
                                color="secondary"
                                onClick={(e) => dispatch(addToCart({ productId: product.id, quantity: 1 }))}
                            >
                                Add To Cart
                            </Button>
                        )}
